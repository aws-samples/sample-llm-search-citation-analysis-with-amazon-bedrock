...
### Epic D — Reliable, parallel keyword research (async, checkpointed, resumable)

- New Step Functions Standard state machine `CitationAnalysis-KeywordResearch` (timeout 30 min) and a non-API worker Lambda `CitationAnalysis-ResearchWorker` (timeout 300 s — allowed because it is not API-backed). The API Lambda `KeywordMgmt` drops back to the 29 s cap, loses `reservedConcurrentExecutions` (it no longer self-invokes) and its documented exception is removed from `lib/citation-analysis-stack.spec.ts`.
- Parallelism (R30): one execution per research job (no shared concurrency cap between jobs); inside a job the `Map ExecuteSteps` state runs one worker invocation per planned query × provider with `maxConcurrency` 10, so an expansion that asks all three web-search providers finishes in the time of the slowest single call instead of the sum. Provider rate limits are respected by the worker's existing retry/backoff; a per-provider concurrency ceiling can be added with a second Map level if a provider starts throttling.
- Job model in the existing `CitationAnalysis-KeywordResearch` table: job row `{id, type, status: pending|running|partial|completed|failed, progress:{steps_total, steps_done, keywords_found}, execution_arn, plan?, created_at, updated_at, ttl (90 days)}` plus one row per step `id = "<job>#step#<n>"` `{step, kind, provider, status, keywords[], raw_response(≤5 KB), error?, started_at, finished_at}`. New GSI `TypeCreatedIndex` (type, created_at) so history is a Query, not a scan.
- States: `MarkRunning` → `Map ExecuteSteps` (each step writes its row on success — partial results survive anything that happens later; Retry ×2 with backoff on `States.TaskFailed/Timeout`; Catch → step marked failed, Map continues) → `Merge` (dedupe by `normalize_keyword`, rank, keep provenance) → `Finalize` (`completed` if all steps ok, `partial` if some failed, `failed` if none produced output). Truncated/invalid LLM JSON is a step failure, never an empty success.
- Worker budget: per-step HTTP timeout 60 s and at most 2 in-process retries; Step Functions owns the outer retries, so the sum can no longer exceed the Lambda timeout.
- API: `POST /keyword-research/expand` and `/competitor` create the job row and `start_execution` (202 with id); new `GET /keyword-research/{id}` (job + steps + merged results); `POST /keyword-research/{id}/retry` (re-runs only failed steps in a new execution, same job id — R17); `/history` uses the GSI and shows progress. The reader-side stale sweep becomes a safety net for `running` jobs older than the state-machine timeout only.
- Frontend `useKeywordResearch`: poll `GET /keyword-research/{id}` every 3 s (backing off to 10 s), render steps as they complete, status chip Running → Processing (x/y) → Completed | Partial (Retry) | Failed (R18); active job id persisted in the URL (`?job=`) and sessionStorage so refresh/tab switch re-attaches (R15). The hook gets a typed client module (`api/research.ts` was an unused duplicate and is removed in Phase 0a; it is re-created here against the new endpoints and actually wired in).
- Quick wins that can ship first as 2.0.1 (see Phase 0): `GET /keyword-research/{id}` by id (removes the scan-visibility loss), TTL on research rows, `retryAttempts: 0` on the self-invoke functions' async config (stops the 3× re-runs), poll window aligned with the server timeout.
