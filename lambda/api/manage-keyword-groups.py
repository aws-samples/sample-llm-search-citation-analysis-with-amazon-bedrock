...
def _scan_all(table: Any, **params: Any) -> list[dict[str, Any]]:
...
def _member_counts() -> dict[str, int]:
    ...
    counts: dict[str, int] = {}
    for item in _scan_all(keywords_table, ProjectionExpression='group_ids'):
...
def _name_taken(name: str, *, exclude_id: str | None = None) -> bool:
    ...
    for item in _scan_all(groups_table, ProjectionExpression='id, name_key'):
        if item.get('name_key') == name_key and item.get('id') != exclude_id:
            return True
    return False


def _validated_name(name: Any, event: dict[str, Any]) -> tuple[str | None, dict[str, Any] | None]:
    if not isinstance(name, str):
        return None, validation_error('name must be a string', event, 'name')
    text = ' '.join(name.split())
    if not text:
        return None, validation_error('name must not be empty', event, 'name')
    if len(text) > MAX_GROUP_NAME_LENGTH:
        return None, validation_error(f'name exceeds maximum length of {MAX_GROUP_NAME_LENGTH} characters', event, 'name')
    return text, None
...
def create_group(event: dict[str, Any], context: Any, body: dict, name: Any, description: str, **_: Any) -> dict[str, Any]:
    ...
    text, error = _validated_name(name, event)
    if error:
        return error
    if _name_taken(text):
...
def update_group(event: dict[str, Any], context: Any, body: dict, name: Any, description: str | None, id: str | None = None, **_: Any) -> dict[str, Any]:
    """PUT /api/keyword-groups/{id}"""
    if not id:
        return validation_error('Group ID is required', event, 'id')
    existing = groups_table.get_item(Key={'id': id}).get('Item')
    if not existing:
...
def delete_group(event: dict[str, Any], context: Any, id: str | None = None, **_: Any) -> dict[str, Any]:
    """DELETE /api/keyword-groups/{id} — also detaches the group from every keyword."""
    if not id:
        return validation_error('Group ID is required', event, 'id')
    if not groups_table.get_item(Key={'id': id}).get('Item'):
        return not_found_response(resource='Keyword group', event=event)
...
def _apply_membership(keyword_id: str, group_id: str, *, add: bool) -> dict[str, Any] | None:
    ...
    if add:
        params['UpdateExpression'] = 'ADD group_ids :gids'
        params['ConditionExpression'] = (
            'attribute_exists(#id) AND (attribute_not_exists(group_ids) OR size(group_ids) < :max)'
        )
        params['ExpressionAttributeValues'][':max'] = MAX_GROUPS_PER_KEYWORD
    else:
        params['UpdateExpression'] = 'DELETE group_ids :gids'
        params['ConditionExpression'] = 'attribute_exists(#id)'
    try:
        return keywords_table.update_item(**params)['Attributes']
...
@parse_json_body
def update_memberships(event: dict[str, Any], context: Any, body: dict, id: str | None = None, **_: Any) -> dict[str, Any]:
...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """Routed entirely by ``route_handler``; see module docstring."""
